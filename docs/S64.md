...
| Mnemonic | Syntax | Semantics | Opcode |
|----------|--------|-----------|--------|
| `JMP`  | `JMP label` | unconditional jump | `0x40` |
| `JEQ`  | `JEQ Rs1, label` | jump if `Rs1 == 1` | `0x41` |
| `JNE`  | `JNE Rs1, label` | jump if `Rs1 == 1` (tests a `NE` result) | `0x42` |
| `CALL` | `CALL Rlink, label` | jump to `label`, write return address into `Rlink` | `0x43` |
| `RET`  | `RET Rlink` | jump to address held in `Rlink` | `0x44` |

By SABI convention, `Rlink` is `R30`:
...
This program has run on `s64emu` and produces `R0=1, R1=2, R2=3, halted=1, fault=0` — see the
roadmap doc for toolchain build status.
